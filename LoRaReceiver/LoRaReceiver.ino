// Only supports SX1276/SX1278
#include <LoRa.h>
#include "LoRaBoards.h"

#ifndef CONFIG_RADIO_FREQ
#define CONFIG_RADIO_FREQ           868.0
#endif
#ifndef CONFIG_RADIO_OUTPUT_POWER
#define CONFIG_RADIO_OUTPUT_POWER   17
#endif
#ifndef CONFIG_RADIO_BW
#define CONFIG_RADIO_BW             125.0
#endif


#if !defined(USING_SX1276) && !defined(USING_SX1278)
#error "LoRa example is only allowed to run SX1276/78. For other RF models, please run examples/RadioLibExamples
#endif

int packetsNums = 100;
int errorPack = 0;
int packetsCounter = 0;
unsigned long lastPacketTime = 0;
const unsigned long timeout = 600;

void setup()
{
    setupBoards();
    // When the power is turned on, a delay is required.
    delay(1500);

    Serial.println("LoRa Receiver");

#ifdef  RADIO_TCXO_ENABLE
    pinMode(RADIO_TCXO_ENABLE, OUTPUT);
    digitalWrite(RADIO_TCXO_ENABLE, HIGH);
#endif

    LoRa.setPins(RADIO_CS_PIN, RADIO_RST_PIN, RADIO_DIO0_PIN);
    if (!LoRa.begin(CONFIG_RADIO_FREQ * 1000000)) {
        Serial.println("Starting LoRa failed!");
        while (1);
    }

    LoRa.setTxPower(CONFIG_RADIO_OUTPUT_POWER);

    LoRa.setSignalBandwidth(CONFIG_RADIO_BW * 1000);

    LoRa.setSpreadingFactor(12);

    LoRa.setPreambleLength(16);

    LoRa.setSyncWord(0xAB);

    LoRa.enableCrc();

    LoRa.disableInvertIQ();

    LoRa.setCodingRate4(7);

    // put the radio into receive mode
    LoRa.receive();

}

void loop()
{

    // try to parse packet
    int packetSize = LoRa.parsePacket();
    if (packetSize) {
        // received a packet
        lastPacketTime = millis();
        //Serial.print("Received packet '");

        String recv = "";
        // read packet
        while (LoRa.available()) {
            recv += (char)LoRa.read();
        }
        if(recv.substring(0, 5) != "hello"){
          errorPack++;
        }

        // Serial.println(recv);

        // print RSSI of packet
        // Serial.print("' with RSSI ");
        Serial.print(LoRa.packetRssi());
        Serial.print(" ");
        Serial.print(LoRa.packetSnr());
        Serial.print("\n");
        if (u8g2) {
            u8g2->clearBuffer();
            char buf[256];
            u8g2->drawStr(0, 12, "Received OK!");
            u8g2->drawStr(0, 26, recv.c_str());
            snprintf(buf, sizeof(buf), "RSSI:%i", LoRa.packetRssi());
            u8g2->drawStr(0, 40, buf);
            snprintf(buf, sizeof(buf), "SNR:%.1f", LoRa.packetSnr());
            u8g2->drawStr(0, 56, buf);
            u8g2->sendBuffer();
        }
        packetsCounter++;
        packetsNums--;
    }
    // else{
    //   if (millis() - lastPacketTime > timeout){
    //      Serial.println("No packets received for 0.5 seconds!");
    //      packetsNums--;
    //      lastPacketTime = millis();
    //   }
    // }
    
    // if (packetsNums == 0){
    //   Serial.println("total packets: ");
    //   Serial.println(packetsCounter);
    //   Serial.println("Error packets: ");
    //   Serial.println(errorPack);
    //   packetsNums = 100;
    //   errorPack = 0;
    //   packetsCounter = 0;
    // }
}
